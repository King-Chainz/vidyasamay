...
import { timetableAPI, timeSlotsAPI } from '../services/api'
...
export default function TimetableViewer() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true)
      const [timetablesRes, timeSlotsRes] = await Promise.all([
        timetableAPI.getAll(),
        timeSlotsAPI.getAll()
      ])
      
      setTimeSlots(timeSlotsRes.data || [])
      await fetchTimetables(timetablesRes.data)
      setError('')
    } catch (err) {
  ...
  const fetchTimetables = async (timetablesData = null) => {
    try {
      if (!timetablesData) {
  ...
  // Generate time slot labels from actual time slots data
  const getTimeSlotLabel = (slotNumber) => {
    const slot = timeSlots.find(s => s.slot_number === slotNumber)
    if (slot) {
      return `${slot.start_time}-${slot.end_time}`
    }
    // Fallback to hardcoded slots if data not loaded
    const fallbackSlots = ['9:00-10:00', '10:00-11:00', '11:00-12:00', '12:00-13:00', '14:00-15:00', '15:00-16:00', '16:00-17:00', '17:00-18:00']
    return fallbackSlots[slotNumber] || `Slot ${slotNumber}`
  }
  
  const slots = Array.from({ length: 8 }, (_, i) => getTimeSlotLabel(i))
