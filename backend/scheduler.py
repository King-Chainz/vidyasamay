...
class TimetableScheduler:
    ...
    def generate_schedules(self, num_solutions=3):
        ...
        for batch in self.batches:
            batch_id = batch['id']
            batch_dept = batch.get('department', '')
            assignments[batch_id] = {}
            
            for subject in self.subjects:
                # Match subjects to batches by department
                subject_dept = subject.get('department', '')
                if subject_dept and batch_dept and subject_dept != batch_dept:
                    continue
                    
                subject_id = subject['id']
                classes_needed = subject.get('hours_per_week', 3)
                    ...
                    for slot in range(self.total_slots):
                        for classroom in self.classrooms:
                            for fac in self.faculty:
                                ...
                                fac_dept = fac.get('department', '')
                                if fac_dept and subject_dept and fac_dept == subject_dept:
                                    var = model.NewBoolVar(f'b{batch_id}_s{subject_id}_sl{slot}_c{classroom["id"]}_f{fac["id"]}')
                                    slot_vars[(slot, classroom['id'], fac['id'])] = var
                    
                    if slot_vars:  # Only add if there are valid assignments
